...
class ValidateStepsProcess extends React.PureComponent {
  ...
  calculateLocalHash = file => {
    this.setState({ localStatus: 'process' });
      ...
      reader.onloadend = () => {
        ...
        ipfsApi
          ...
          .then(response => {
            ...
            console.log(ipfsId);
            this.setState({ localStatus: 'finish', ipfsHash: ipfsId });
            resolve(ipfsId);
          })
          .catch(err => {
  ...
  requestRemoteHash = () => {
    this.setState({ remoteStatus: 'process' });
    return new Promise((resolve, reject) => {
      axios
        ...
        .then(response => {
          if (response.data.length > 0) {
            this.setState({ remoteStatus: 'finish' });
  ...
  verifySignature = async () => {
    ...
    this.setState({ validationStatus: 'process' });
    return new Promise((resolve, reject) => {
      axios
        ...
        .then(response => {
          const { data } = response;
          if (data.valid) {
            this.setState({ validationStatus: 'finish' });
            resolve();
          } else {
  ...
  verityStatus = certificate => {
    this.setState({ certificateStatus: 'process' });
    return new Promise((resolve, reject) => {
      if (certificate.valid) {
        this.setState({ certificateStatus: 'finish' });
        resolve();
      } else {
        this.setState({ certificateStatus: 'error' });
        reject();
      }
    });
  };
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <Row type="flex" justify="center" align="center">
          {status === 'error' && <h1>Certificado Inválido</h1>}
        </Row>
